...
3) List all aliases
4) Delete all aliases

This feature is backed by an in-memory database implemented as a Java `HashMap<String, Command>`.
A HashMap is chosen for several reasons:
* Adding and removing an alias is straightforward (using in-built Java API) and efficient (`O(1)` time complexity)
* Checking whether an alias exists is fast (`O(1)` performance)
* HashMaps naturally facilitate the process of looking up an associated value with a given key
* Java HashMaps has built-in operations to facilitate the listing of all aliases, or the deletion of all aliases

*Alternative*: An alternative implementation could use a Java `ArrayList`. However, that adds additional code complexity,
as there needs to be a way of associating a `String`-type alias with a `Command` object.
For instance, we could create an `ArrayList<AliasWrapper>`, where `AliasWrapper` is a wrapper class to associate an alias with a Command.
That is, however, inelegant and inefficient, as opposed to a `HashMap` solution.
...
A meta-command is one that pertains to AliasManager (and by association SourceManagerParser), i.e. `alias`, `alias-rm`, `alias-ls`, etc.
...
For instance, `alias FOO BAR` is delegated to the AliasAddMetaCommandParser (a concrete subclass of AliasMetaCommandParser).
The AliasMetaCommandParser concrete subclasses parses user inputs, potentially throwing exceptions, and finally returns a DummyCommand response object.

In normal operation, when the user uses an alias, SourceManagerParser parses user input to extract the "command word".
