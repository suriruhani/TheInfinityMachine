...
It extends `Infinity Machine` with an find feature, allowing user to search through source entries by the title, type, detail and/or tags.
...
(trimmed by space, case insensitive) is implemented. It is split by the CLI prefixes and implements conjunction logic, by only returning `true`
for those sources that satsify all the constraints, i.e. have all the fields matching as entered by the user.
...
Step 4. The user executes `search t/CS2103` and all those sources that have 'CS2103' as one of their tags are enumerated.
