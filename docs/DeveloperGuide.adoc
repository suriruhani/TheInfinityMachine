...
The command alias feature allows users to use shorthand commands to rapidly "get things done", for instance using `a` instead of `add`.
...
* Adding and removing an alias is simple
* Checking whether an alias exists is fast (`O(1)` performance, as opposed to `O(N)` in an unsorted list)
* HashMaps naturally facilitate the process of looking up an associated value with a given key
* HashMaps has built-in operations to facilitate the listing of all aliases, or the deletion of all aliases
...
That is, however, inelegant and contrived, as opposed to a `HashMap` solution.
Furthermore, checking for membership in an `ArrayList` is an O(N) operation.

*Operation*: User input is parsed in such a way that the "command" word is extracted and checked to ascertain if it is an
"original command", or a "command alias". This is implemented simply by augmenting the existing logic (which checks for
original commands) with an additional `default` block, such that if the command word is not an original command, we check if
it is a registered alias of a command.

If so, we run the command by looking up the `Command` object in our `HashMap`.

Otherwise, it is a situation of invalid user input.
...
(For all use cases below, the *System* is the `The Infinity Machine` and the *Actor* is the `user`, unless specified otherwise)
...
* 1b. The command entered is invalid. Either it is spelled wrongly or does not have the correct arguments and parameters.
+
[none]
...
* 3b. The command entered is invalid. Either it is spelled wrongly or does not have the correct arguments and parameters.
+
[none]
...
* 1a. The command entered is invalid. Either it is spelled wrongly or does not have any keywords.
+
[none]
...
* 1a. The command entered is invalid. The command was spelt wrongly.
+
[none]
** 1a1. The Infinity Machine shows an error message together with a help text to guide the user along.
+
...
* 3c. The command entered is invalid. Either it is spelled wrongly or does not have the correct arguments and parameters.
+
[none]
...
* 1a. The command entered is invalid. Either the command was spelt wrongly or the parameters entered were incorrect.
+
[none]
** 1a1. The Infinity Machine shows an error message together with a help text to guide the user along.
+
...
* 1a. The command entered is invalid. The command is spelt wrongly.
+
[none]
** 1a1. The Infinity Machine shows an error message together with a help text to guide the user along.
+
