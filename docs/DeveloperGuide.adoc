...
(trimmed by space, case insensitive) is implemented. It is split by the CLI prefixes and implements conjunction logic.
...
* **Alternative 1 (current choice):** Runs through all entries and matches the arguments, field by field.
** Pros: Easy to implement.
** Cons: May have performance issues in terms of time usage.
