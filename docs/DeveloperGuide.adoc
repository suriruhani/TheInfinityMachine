...
* Adding and removing an alias is straightforward (using Java HashMap API) and efficient (in `O(1)` time)
* Checking whether an alias exists is efficient (in `O(1)` time)
* HashMaps naturally facilitate the process of looking up an associated value of a given key
...
On the other hand, meta-commands operate on AliasManager.
Therefore, it is desirable to draw a distinction between regular commands and meta-commands throughout the codebase.

To implement aliasing, we created the AliasManager interface to practice design by contract.
AliasManager is command-agnostic.
It operates through its API, and is not concerned with the choice of meta-commands selected (e.g. modifying `alias` to `alias-add`).
We also created a class ConcreteAliasManager to implement the AliasManager interface.

As for SourceManagerParser, we created an alternative constructor to accept an AliasManager object for dependency injection.
...
However, working through an interface (and implementing an alternative constructor) provides the flexibility to swap out ConcreteAliasManager for an alternative AliasManager implementation, such as a stub, for unit testing. This improves testability and maintainability.
...
Therefore, for this purpose, we created a class `DummyCommand` which does nothing except return a CommandResult object to display feedback to the user.
...
This makes for a more reusable component and improves testability as well as maintainability.
...
By creating an interface, we avoid a situation of circular dependency where by both components are tightly coupled to each other.
...
image::pinUnpinStructure.png[width="800"]
