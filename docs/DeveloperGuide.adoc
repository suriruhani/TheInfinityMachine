...
==== Overview
It extends `Infinity Machine` with a recycle bin feature, allowing user to delete and restore source from a persistent deleted sources list.
...
* A listener has also been added in the `LogicManager` so that operations
egarding the Recycle Bin can be detected accordingly.
