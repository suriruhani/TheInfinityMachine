...
Searches for entries amongst the list of sources by title based on the user input. +
Finds all entries with a matching title as entered by the user. It is case insensitive (eg. `RSAalgorithm` is same as `rsaalgorithm`) +
Format: `search title`
