...
Another limitation of the AB4 implementation is that it tests for exact (case insensitive) matching between the keywords
entered by the user and the field values of sources. This is very intuitive because this would expect the user to remember and
recall the exact tag, title, type or detail they had entered when creating the source (or after editing). This is especially useless
when the user stores his entire research data in the `detail`, say 3 paragraphs, now to reproduce the source he must enter those 3 paragraphs
